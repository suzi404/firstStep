...
angular.module('tm.pagination', []).directive('tmPagination',[function(){
    return {
        ...
        template: '<div class="page-list">' +
            ...
            '<span ng-if="item == conf.currentPage" class="active" >{{ item }}</span>' +
            '<span ng-if="item == \'...\'" class="separate" >{{ item }}</span>' +
            '<span ng-if="item != conf.currentPage && item != \'...\'" >{{ item }}</span>' +
            //modify by ck end
            '</li>' +
            ...
            '</div>'+
            //'<div class="no-items" ng-show="conf.totalItems <= 0" style="width: 100px"><img src="images/nomessage.png">查询不到数据</div>' +
            '</div>',
        replace: true,
        scope: {
            conf: '='
